...
class _RawContentCache {
  static final Map<String, g.Content> _cache = {};

  static String _key(List<AIChatMessageToolCall> calls) {
    return calls.map((c) => '${c.name}:${c.argumentsRaw}').join('|');
  }

  static void store(List<AIChatMessageToolCall> calls, g.Content content) {
    _cache[_key(calls)] = content;
  }
...
extension ChatMessagesMapper on List<ChatMessage> {
  ...
  g.Content _mapAIChatMessage(final AIChatMessage msg) {
    // Use cached raw content if available (preserves ThoughtSignaturePart
    // for Gemini thinking models)
    if (msg.toolCalls.isNotEmpty) {
      final cached = _RawContentCache.retrieve(msg.toolCalls);
...
extension GenerateContentResponseMapper on g.GenerateContentResponse {
  ChatResult toChatResult(final String id, final String model) {
    ...
    // Cache raw content for thinking model thought_signature preservation
    if (toolCalls.isNotEmpty && candidate.content != null) {
      _RawContentCache.store(toolCalls, candidate.content!);
    }
  ...
  FinishReason _mapFinishReason(final g.FinishReason? reason) =>
      switch (reason) {
        ...
        g.FinishReason.language => FinishReason.unspecified,
